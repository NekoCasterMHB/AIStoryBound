...
- 音色清单与「旁白+引号对白」多音色分段在 `shared/tts.ts`(纯函数 `buildSpeechSegments`;说话人归属 = 引号前 60 字内 lastIndexOf 最大的角色名,归属失败沿用上一说话人,未配置音色的角色回退旁白音色)。
...
- **打字机跟读**:typewriter 新增 `waitFor` 显示门(`app/utils/typewriter.ts`)——每 tick 显示前调用,返回 pending Promise 即暂停显示;游戏页在门里实现「段落完整上屏 → 播该段音频(播完放行)→ 继续显示」,仅在 `narrReady`(叙事流结束、全文稳定)后才预取,失败推进游标不阻塞文字;快进/停止回合时 `ttsPlayer.stopTts()`。

...
async function confirmBuy() {
  ...
  try {
    ...
    toast.add({
      ...
      description: res.price > 0 ? '购买后永久可下载;发布者将获得其中的 80%,收益在个人中心领取后到账' : '免费 Skill 在商城拥有更高展示优先级',
      color: 'success'
    })
...
const priceTarget = ref<MyPublishedSkill | null>(null)
const priceDraft = ref('')
const priceSaving = ref(false)

function openPriceModal(p: MyPublishedSkill) {
  priceTarget.value = p
...
async function savePrice() {
  ...
  try {
    await $fetch(`/api/store/skills/${p.id}/price`, { method: 'POST', body: { price: n } })
    toast.add({
      title: '售价已更新',
      description: `「${p.name}」新售价 ${n.toLocaleString()} token,仅影响之后的购买;已购者不受影响`,
      color: 'success'
    })
    priceModalOpen.value = false
    await loadMine()
        ...
        <template v-if="buyStage === 'confirm'">
          <template v-if="buyTarget && buyTarget.price > 0">
            <p class="text-sm text-neutral-600 dark:text-neutral-400">
              ...
              获取该 Skill,购买后永久可下载。发布者将获得售价的 80%,20% 为平台手续费;收益进入发布者个人中心「收益」,领取后到账。
            </p>
    ...
    >
      <template #body>
        ...
        <p class="mt-2 text-xs text-neutral-400">
          仅影响之后的购买;已购者不受影响,不涉及退款。0 表示免费,免费 Skill 将获得更高展示优先级。
        </p>
      </template>
      <template #footer>

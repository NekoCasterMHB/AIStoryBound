...
// 多连接:每连接调用 createWebBluetoothTransport() 获得独立实例(各自 currentChar/断连回调);
// 已授权设备缓存与电量缓存按设备 id 全局共享(与连接实例无关)。
...
// 已授权设备与电量按设备 id 全局共享(与连接实例无关;多实例各自只持有自己的当前连接特征)
...
/** 创建独立 Web Bluetooth 传输实例(每连接独立 currentChar / 断连回调;扫描记住的所选设备随实例) */
...
/** 全局单例(兼容旧调用方;多连接请用 createWebBluetoothTransport 各建实例) */
export const webBluetoothTransport: ToyTransport = createWebBluetoothTransport()

 ...
 * 指令数量单回合上限 16 条(超出静默丢弃),持续性互动应使用 wave 而非连发 dev;指令不可见、不得在正文复述。
 * 调教模式(wave)对任意可调强度的功能统一可用。
 * 多设备:多个设备同时连接时,指令带插件 id 前缀路由到目标设备(如 [[dev:sosexy:suction:80:2:5]]);
 * 仅一台连接时可省略前缀(缺省路由到当前连接)。能力清单按设备逐条给出,方便 AI 按 id 选择。
 */
...
- [[dev:功能id:强度[:模式[:持续秒数]]]] 或 [[dev:插件id:功能id:强度[:模式[:持续秒数]]]] — 文字显示到该句时设备立即执行,如 [[dev:vibration:80:2:5]](震动强度80/模式2/持续5s;持续秒数省略=保持,由后续指令或自动停止接管);多台设备已连接时必须带插件 id 前缀(如 [[dev:sosexy:suction:80:2:5]])路由到目标设备;
...
可用设备:\n${lines}\n指令中的功能id 必须用清单括号内标注的功能id(如 electric),不要用中文名代替;强度必须在该能力声明的范围内取值;mode 为该功能支持的档位;duration 为持续秒数(到时设备自动停止,上限 3600 秒)。单回合指令最多 16 条,超出会被静默丢弃;持续的强度起伏互动请用 [[wave]],不要连发 [[dev]];未连接设备的事件会被拒绝。没有设备互动情节就不埋指令。`
}
...
export function buildTurnPromptParts(args: TurnPromptArgs): TurnPromptPart[] {
  ...
  const baseRules = [
    `你只能以玩家角色「${playerName}」的第一视角与内心展开场景,用旁白叙事推进;对话行以「角色名:」开头,非玩家角色可自由说话/行动。角色对白格式(必须严格遵守):每段角色台词用直角引号「」包裹,引号前紧邻说话角色名与引导词(如:掌柜压低了声音:「……」、青璃低声道:「……」);一个引号内只放同一角色的连续台词,禁止在引号内混入旁白描述或其他角色的插话;旁白叙述本身不要加引号。严格禁止切换到其他角色(尤其原著主角)的视角,禁止替其他角色做决定、禁止描写其他角色的内心活动——其他角色的反应只能通过他们的言行从外部呈现。`,
    `每一处引号对白(包括同一角色连续多次开口)都必须在引号前紧邻标注「说话角色名+引导词+冒号」,禁止任何无署名的引号对白;引导语中只允许出现说话角色本人的名字与动作,禁止「A看着B道:」这类引导语夹带其他角色名的句式——需要交代动作对象时放在引导语之前单独成句(如:林凡看着小美。小美低声道:「……」)。引号前紧邻冒号的角色名就是对白配音的说话人依据,标注错误会导致配音归错角色。`,

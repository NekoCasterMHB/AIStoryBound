// server/api/store/novels/[id]/status.post.ts
// 发布者管理商品上下架(仅限本人商品):
// - status=removed 下架:仅已上架(approved)商品可下架,下架后从书架商城隐藏,已购者仍可下载既有版本;
// - status=pending  重新上架:仅已下架商品可提交,进入待审核,管理员审核通过后恢复展示。
...
export default defineEventHandler(async (event) => {
  ...
  const rows = await db.select({ id: novelProducts.id, status: novelProducts.status })
    .from(novelProducts)

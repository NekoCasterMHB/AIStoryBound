...
// 缓存管理(管理端):把单条缓存成书的源 txt「添加到推荐书架」。
// 源 txt 复制到 R2 预置区(preset-txt/<id>.txt),并插入 preset_novels 记录(featured=1);
// 预置下载接口在静态资源缺失时回退读 R2。预置世界(0 token 直接进入)已下线,不再复制成书文件。
...
export default defineEventHandler(async (event) => {
  ...
  // 源 txt(与缓存同 hash 的全站共享原文)已存在 R2,复制到预置区
  ...
  await bucket.put(txtKey, sourceBytes)

  // 字数取源 txt 长度(推荐书架卡片显示字数);description 暂无成书可取,置空
  let charCount = 0
  try {

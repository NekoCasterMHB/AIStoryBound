...
if (!outTradeNo) { console.error('用法: node scripts/query-order.mjs <out_trade_no>'); process.exit(1) }
...
function buildSignStr(params) {
  return Object.keys(params).filter(k => k !== 'sign' && k !== 'sign_type').filter(k => params[k] !== undefined && params[k] !== null && params[k] !== '').sort().map(k => `${k}=${params[k]}`).join('&')
}
